...
- Add `Assiette::ReferenceLog` and `AssetHandler#reference_log`, remembering what each page linked the last time this process rendered it. Rails evaluates `etag` blocks before the template renders, so the current render's links do not exist yet when the validator is built — the previous render's set is what stands in. A page this process has not rendered yet has nothing remembered and falls back to `AssetHandler#digest`.
